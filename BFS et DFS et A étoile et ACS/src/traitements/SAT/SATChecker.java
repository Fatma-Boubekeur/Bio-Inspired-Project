...
//A étoile  
    ...
    public boolean check(CheckMethod method, long timeLimit) throws Exception { 
        ...
        else if(method==CheckMethod.BFS) result= bfsCheck();
        else {
...
/****************************************************************************************************************************************************/   
    ...
    private boolean[] acsCheck(double alpha, double beta, double q0, double vaporation){
        ...
        for (int i = 0; i < pheromone.length; i++)
            pheromone[i][0]=pheromone[i][1]= p0;

        Ant[] antTable= new Ant[variables.length];
        ...
        while(iteration++<ITERATIONS){
            for (int i = 0; i < antTable.length; i++) {
                ...
                //System.out.println(antTable[i].hashCode());
...
/**
 * class StackVariable:
 * used to hold a stack element to simulate the tree search using an iterative approach.
 * a stack element contains the variable, its value and the depth it appears in to handle jumps between branches.
**/
class StackVariable{
